...
However, if the child looks well, but the clinician determines that the febrile infant is not in the low risk group for a UTI, the AAP recommends two options. One option is to obtain the urine sample using an invasive method.
...
An open-access study published this year in the BMJ suggests a new method called the Quick-Wee could speed up collection of clean catch urine samples in patients for whom we are considering a diagnosis of UTI.
...
In addition, the researchers assessing the data could have been blinded to which intervention group each data set came from. This is the simplest way of introducing blinding into a study but it is rarely utilized in isolation – and perhaps it should be used more.
